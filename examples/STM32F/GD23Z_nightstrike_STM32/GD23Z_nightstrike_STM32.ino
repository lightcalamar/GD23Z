...
//SD STM32
SdFat SD(2);  const uint8_t SD2_CS = PA11;   // chip select for sd2
//SD STM32
...
void draw_dxt1(byte color_handle, byte bit_handle)
{
  ...
  GD.SaveContext();
     ...
     GD.Begin(RECTS);
     GD.Vertex2f((480) * 16, 0* 16);   GD.Vertex2f((800) * 16, (100) * 16);
     GD.Vertex2f((0) * 16, 272* 16);   GD.Vertex2f((480) * 16, (320) * 16);
  GD.RestoreContext();
  //FT813 5" 800x480
  }
}
...
public:
  ...
  void load_level(byte n) {
    ...
    //GD.loadSDIO(levelRes);  //T36
    GD.loadSDIO(levelRes);  //SD STM32
    //GD.load(filename);
    //End RndMnkIII
...
void setup()
{
  ...
  //SD.begin();   //T36
  SD.begin(SD2_CS, SD_SCK_MHZ(FreqSD2)); //SD STM32 
  SD.chvol();
